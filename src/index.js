...
function loadCities(contacts) {
  const selectMenu = document.querySelector("#filterOptions");
  ...
  const content = citiesList.map(renderCity).join("");

  selectMenu.innerHTML = `<option value="0">-- Select a city --</option>${content}`;
}
...
function main() {
  ...
  // add an event listener to all of main
