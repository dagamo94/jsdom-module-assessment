...
/*
  Add a `change` event listener to the `filterOptions` select element.
...
function filterHandler(event) {
  ...
  selection == 0 ? render(contacts) : render(filterByCity(selection));
}
/*
...
function renderCity(city) {
  var content = "\n  <option value=\"".concat(city, "\">").concat(city, "</option>");
  return content;
}

function loadCities(contacts) {
  var selectMenu = document.querySelector("#filterOptions");
  var cities = contacts.map(function (contact) {
    return contact.address.city;
  });
  var citiesSet = new Set(cities);
  var citiesList = Array.from(citiesSet);
  var content = citiesList.map(renderCity).join("");
  selectMenu.innerHTML = "<option value=\"0\">-- Select a city --</option>".concat(content);
}
/*
...
function main() {
  // *** Filter by City ***
  var cityFilter = document.querySelector("#filterOptions");
  cityFilter.addEventListener("change", filterHandler); // *** Delete contacts ***
  // Select all of 'main' to simultaneously select all the buttons inside of it

  var main = document.querySelector("main"); // add an event listener to all of main

  main.addEventListener("click", deleteButtonHandler); // *** Render to page ***
